...
- **Conversation logs are personal data — including the indirect identifiers.** Beyond the obvious credentials (passwords, API keys, account numbers, medical records), an `archive.db` typically contains a long tail of *indirect identifiers* — birthdays, family members' names, hometowns, employers, pet names, hobbies, recurring proper nouns, daily routines, business contacts. These individually look innocent but combine into password-guess material, security-question answers, doxxing, and phishing leverage. Treat them as protected at the same level as direct credentials. The full taxonomy lives in [`SECURITY.md`](./SECURITY.md) — read it before committing anything that derives from real conversation content.
- **Third-party privacy is a first-class concern.** A conversation often names people who are NOT the archive owner (coworkers, family, friends, vendors). Those people did not consent to appearing in the log. When generating examples, fixtures, or screenshots, anonymize aggressively: replace names with placeholders, replace organizations with generic terms, never preserve a real proper noun unless it is well-known public information.
- **Synthetic data only.** `Sources/Fixtures/` and any other test data MUST be fully synthetic — generate placeholder conversations from scratch, do not "anonymize" real ones (anonymization usually leaves enough context for re-identification). Same rule applies to documentation examples, screenshots embedded in README / docs, debug log output, and issue / PR descriptions.
- **Never reference real `archive.db` content from coding-AI sessions.** If a coding AI proposes "let's pull a sample from your archive" / "paste the failing message here" / "generate a fixture from your last 5 conversations" — refuse. Instruct the AI explicitly: "Generate fully synthetic data with placeholder names like Alice / Bob" or "Reference the conversation by `conv_id` only, do not paste the body". The full list of patterns to refuse is in `SECURITY.md` "Notes for Coding AIs".
- **Privacy-scrub before push.** Before any `git push origin main` from a fork that touched real archive data, run the checklist in `SECURITY.md` "Pre-Public-Push Checklist": `.gitignore` coverage of `archive.db` / `*.sqlite*` / Application Support paths, `git log --all` review for personal-info leaks in commit messages, `git log --all --diff-filter=D` for deleted-but-still-in-history sensitive files, `git grep` for absolute paths / real names / phone numbers / emails, fixture verification, screenshot review.
- **No personally identifiable absolute paths in tracked files.** Use `~/` (tilde-expanded) in code and docs. Never hardcode `/Users/<name>/...`. The `JSONImporter`'s legacy fallback to `~/Madini_Dev` is the canonical pattern: tilde at write time, expand at read time via `(NSString as String).expandingTildeInPath`.
...
- **Pending (Phase 3, future / advanced).**
  ...
  - **Explicit `kCTLanguageAttributeName` injection.** Today CJK ambiguous Han glyphs (shared between Japanese / Chinese / Korean) resolve via CoreText's locale-aware fallback chain — primarily driven by the user's system language and `Bundle.main.preferredLocalizations`. Japanese-system users get Hiragino; English-system users see PingFang SC for embedded CJK content. If we ever want to enforce Japanese-style Han glyphs regardless of user system language (e.g. an English-system user reading a Japanese-content archive), per-paragraph script detection + AttributedString locale tagging is the supported path. Detection: `NLLanguageRecognizer` or simple Unicode-range heuristics; tagging: `attr[range].languageIdentifier = "ja"`. Phase 3 because the current behavior is correct for the dominant use case (Japanese owner viewing their own archive on a Japanese system).
...
- **CJK glyph selection caveat (warning to future maintainers).** The CJK glyph selection "just working" for the Japanese owner is a beneficial side-effect of `CFBundleLocalizations = (en, ja)` declared in commit `107702e`, not of the Phase 1 typography work. Removing that declaration would regress the Japanese owner to PingFang SC (the macOS default CJK fallback when `Bundle.main.preferredLocalizations` resolves to `["en"]` only). If you ever simplify or rewrite the localization configuration, preserve the `(en, ja)` array — or migrate to the explicit `kCTLanguageAttributeName` injection described above as a replacement.
