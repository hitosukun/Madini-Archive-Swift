...
## Window Model
- **Madini is single-window by design.** The main conversation surface (`WindowGroup` in `MadiniArchiveApp.swift`) is intended to run as exactly one window at a time. File > "New Window" / ⌘N is explicitly disabled by replacing SwiftUI's `.newItem` command group; the replacement publishes a single File > "Close" (⌘W) item so the File menu still appears in the menu bar with the canonical close action visible. Hiding the File menu entirely (which is what an empty `.newItem` replacement would do, since SwiftUI collapses File when `.newItem` is empty) is incongruous with macOS HIG and unfriendly to accessibility tooling — keeping Close visible is the explicit-over-implicit counterpart to disabling ⌘N.
- **Known cosmetic redundancy in the File menu.** Once `.newItem` is non-empty, AppKit re-injects its standard "Close" and "Close All" items alongside our custom Close. SwiftUI offers no public hook to suppress those (verified empirically: `.commandsRemoved()`, hidden anchor Buttons, and Dividers all leave the AppKit pair in place; only collapsing the entire File menu hides them). The three items all dispatch to `NSWindow.performClose(_:)` on the key window, so the duplication is functionally inert. Suppressing it would require post-launch NSMenu surgery in `MadiniAppDelegate`, which the project deliberately avoids — minimal AppKit interop is preferred over fragile menu mutation.
