...
- user（オーナー）が読み飛ばせるよう、折りたたみブロックにまとめる
...
オーナーの記憶にある「ソース × モデル別レンダリング切替」は `MessageRenderProfile` のこと。設計は放棄されたものではなく、**活発に使われている（ただし main にはまだ来ていない）**。新規フレームワーク設計は、この既存実装を土台にして発展させるのが適切。

---

## 3. 現象の精密な記述

> **Note**: 以下の Bug A / Bug B / 追加発見セクションは、**実会話の例を合成例に置換した公開版**。
> 元の調査は実 archive.db の特定会話を対象に行ったが、本リポジトリ公開のため
> 会話タイトル・UUID・本文 verbatim 引用は構造的に等価な合成サンプルに差し替え済み。
> 技術的論点（block 構造、parser の挙動、誤判定の root cause）はそのまま保持。

### 3.1 Bug A — 数式の言語誤判定

**対象**: Conversation A — 数式（距離関数の公理）を番号付きリストで列挙した日本語主体の Claude 会話、特定の assistant message。
...
**対象**: Conversation B — 日本語主体の Claude 会話、assistant が応答前に英語の内的独白を出すケース。Claude export の `chat_messages[].content[]` 配列内に `type: "thinking"` ブロックを含む。

**raw export JSON 構造（合成例、構造は実データと同型）**:
...
```json
{
  ...
  "content": [
    { "type": "text", "text": " " },
    { "type": "thinking",
      "thinking": "[English thinking preamble — model reasoning about how to respond, ~200 chars]" },
    { "type": "text",
      "text": "[Japanese response body, ~300 chars including markdown headings and lists]" }
  ]
}
```

**Python importer 後の `messages.content`** —
`message.text`（Claude export がフラット化済みのテキストフィールド）を採用するため、上記 thinking と text が**結合された一本の文字列**として `messages.content` 列に格納される。

```
[English thinking preamble]
[Japanese response body continues...]
```
...
**サンプル**: Conversation C — Conversation B と同じ Claude 会話形態だが、thinking が**日本語**で書かれているケース。

**raw export JSON 構造（合成例）**:
...
```json
{
  ...
  "content": [
    { "type": "thinking",
      "thinking": "[Japanese thinking preamble, e.g. 'ユーザーは…について尋ねています。…の意味だと思われます。…で応答します。']" },
    { "type": "text",
      "text": "[Japanese response body]" }
  ]
}
```
...
8件の Claude ソース会話をサンプリングした結果（実会話への参照は匿名化）:

| 会話（匿名） | monologue 言語 | monologue パターン | raw JSON での分離 |
|------|---------------|-------------------|-------------------|
| Conversation B | 英語 | "The user wants me to…" 形式 | `type: "thinking"` ✓ |
| Conversation A | なし | — | — |
| Conversation C | 日本語 | "ユーザーは…と尋ねています" 形式 | `type: "thinking"` ✓ |
| Conversation D | 英語 | "The user wants me to…" / "I need to…" 形式 | `type: "thinking"` ✓ |
...
3. 設定として宣言的（オーナーがコードを読まずに挙動を把握できる）
...
- ユーザー（オーナー）による per-conversation オーバーライドが必要になった場合のみ、view-layer schema に `rendering_overrides` テーブルを追加
...
`MessageRenderProfile` と改良版 `ForeignLanguageGrouping` は `vault/phase-c-importer-audit` ブランチにある。これらを main にマージすることが Phase 0 の前提条件。phase 系ブランチ群のマージ戦略（一括 or 段階的）はオーナーの判断が必要。
...
推奨: コード内定数（`MessageRenderProfile.resolve()`）。per-conversation オーバーライドが必要になるまで DB/UserDefaults は不要。オーナーが「特定の会話だけ thinking を表示したい」等の要望を持っているかで判断。
...
どの戦略を取るかはオーナーの判断。(a) は表示切替 UI が必要、(b) は Python 側の作業、(c) は信頼性に課題。
